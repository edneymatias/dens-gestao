...
Este documento é o checklist autoritativo do trabalho que estamos fazendo — inclui o que já foi realizado, o raciocínio por trás das decisões e os próximos passos. Siga-o passo-a-passo; cada etapa pequena deve ser concluída e marcada como "RESOLVIDO" antes de avançar.
...
- [RESOLVIDO] Usuários com ULID (PK)
  ...
  - Comandos executados:
    ...
    - Verificação via Tinker: `
      \App\Models\User::factory()->create()->id` → ULID (26 chars)
  - Racional: ULIDs são ordenáveis, seguros para exposição em URLs e uniformes entre tenants.
...
1. (13.2B) Comando `tenant:provision` — criar tenant, gerar db_name seguro, criar DB e rodar migrations do tenant (PRIORITÁRIO)
...
3. (13.4) Ajustar tabela `domains` para referenciar tenants por ULID (se aplicável)
...
4. (13.5) Middleware de inicialização de tenancy por sessão
...
5. (13.6) Tenant selection flow (UI + controller)
...
6. (13.7) Jobs / Queues: padronizar tenant context in background jobs
...
7. (13.8) Filament 4 + Filament Shield integration
...
8. (13.9) owen-it/laravel-auditing
...
Se escolher **Gerar comando provision**, eu te entrego o arquivo completo do comando artisan `tenant:provision` pronto para colar em `app/Console/Commands/TenantProvision.php`. Ele irá:
